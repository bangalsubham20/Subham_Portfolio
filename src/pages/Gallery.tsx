...
export default function Gallery() {
  ...
  useLayoutEffect(() => {
    ...
    let ctx = gsap.context(() => {
      ...
      if (title) {
        // Only split if not already split (though checking innerHTML structure is tricky, 
        // relying on SplitText component usually handles this better, but here we do manual)
        const spans = title.querySelectorAll('span');
        spans.forEach(span => {
          if (span.textContent && !span.querySelector('.char')) {
  ...
  // Handlers for Gallery Item hover effects (micro-interactions)
  const handleItemEnter = (e: React.MouseEvent<HTMLDivElement>, id: number, rotation: number) => {
    setHoveredImage(id);
    handleMouseEnter(); // Trigger cursor effect
  ...
  const handleItemLeave = (e: React.MouseEvent<HTMLDivElement>, rotation: number) => {
    setHoveredImage(null);
    handleMouseLeave(); // Reset cursor effect
          ...
          <section className="gallery-container relative min-h-[575vh] lg:min-h-[275vh] py-24">
            {images.map((img) => (
              <div
                ...
                onMouseEnter={(e) => handleItemEnter(e, img.id, img.rotation)}
                onMouseLeave={(e) => handleItemLeave(e, img.rotation)}
              >
                <img
                  src={img.url}
